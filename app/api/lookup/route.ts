...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!result) return Response.json({ error: "This Florida ZIP code is not in our verified pilot area yet." }, { status: 404 });
    const providers = result.providers;
    return Response.json({ zipCode: result.zipCode, city: result.city, county: result.county, state: result.state,
      stateName: result.stateName, status: result.status, confidenceStatus: result.confidenceStatus, jurisdictionNotes: result.jurisdictionNotes, lastUpdated: result.lastUpdated, disclaimer: result.disclaimer, providers: {
