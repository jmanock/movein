...
export async function POST(request: Request) {
  ...
  try {
    getDatabase().prepare(`INSERT INTO correction_submissions
      (zip_code, provider_category, provider_name, issue_details, source_url, reply_email)
      VALUES (?, ?, ?, ?, ?, ?)`).run(data.zipCode, data.category, data.providerName, data.details, data.sourceUrl || null, data.replyEmail || null);
