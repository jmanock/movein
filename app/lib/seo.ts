...
export function isZipResultIndexable(result: LookupResult | null): result is LookupResult {
...
export function getIndexableZipResults() {
  return getActiveZipCodes().map((zip) => { try { return getLookupResult(zip); } catch { return null; } }).filter(isZipResultIndexable);
}
