...
export default function FaqPage() { return <main id="main-content"><JsonLd data={schema} /><div className="shell page-breadcrumb-wrap"><Breadcrumbs items={[{ label: "Home", href: "/" }, { label: "FAQ" }]} /></div><PageHero eyebrow="FAQ" title="Straight answers about finding local services." description="ZIP codes are useful starting points, but the exact address decides many services." /><section className="section"><div className="shell faq-layout"><nav aria-label="FAQ categories"><span>Jump to</span>{groups.map((group) => <a href={`#${group.id}`} key={group.id}>{group.label}</a>)}</nav><div>{groups.map((group) => <section className="faq-group" id={group.id} key={group.id}><h2>{group.label}</h2><div className="faq-list">{group.items.map(([question, answer]) => <details key={question} data-analytics-event="faq_interaction"><summary>{question}</summary><p>{answer}</p></details>)}</div></section>)}</div></div></section></main>; }
