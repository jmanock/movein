...
export default async function ZipResultPage({ params }: { params: Promise<{ zip: string }> }) {
  ...
  return <main id="main-content"><JsonLd data={schema} /><div className="shell result-page" data-analytics-county={result.county ?? undefined}><Breadcrumbs items={[{ label: "Home", href: "/" }, { label: "Learn Your Area", href: "/learn-your-area" }, { label: `ZIP Code ${zip}` }]} /><LookupResults result={result} relatedAreas={relatedAreas} /><section className="search-again"><h2>Check another ZIP code</h2><ZipLookupForm compact /></section></div></main>;
}

function UnsupportedZip({ zip }: { zip: string }) {
  return <main id="main-content"><div className="shell unsupported-result"><span className="status-icon"><MapPin size={27} aria-hidden="true" /></span><span className="eyebrow">Coverage is still growing</span><h1>We do not have a reviewed result for {zip} yet.</h1><p>That does not mean service is unavailable. It means MoveIn has not verified enough official information to publish a provider shortlist for this ZIP.</p><div className="unsupported-actions"><Link className="button" href="/coverage">See current coverage <ArrowRight size={17} /></Link><Link href="/resources/set-up-utilities">Use the utility setup guide</Link></div><section><h2>Useful ways to continue</h2><ul><li>Check your city or county utility department for water, sewer, and trash.</li><li>Use the Florida Public Service Commission utility finder for regulated electric and gas service.</li><li>Check internet availability using your complete address on the FCC National Broadband Map.</li></ul></section><div className="search-again"><h2>Try another ZIP code</h2><ZipLookupForm compact initialZip={zip} context="unsupported_zip" /></div></div></main>;
}
