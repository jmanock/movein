...
import { isValidZip } from "../../db/lookup";
...
export default async function LookupPage({ searchParams }: { searchParams: Promise<{ zip?: string }> }) {
  const { zip } = await searchParams;
  if (zip && isValidZip(zip)) redirect(`/lookup/${zip}`);
