...
export const metadata: Metadata = pageMetadata("Moving and Utility Setup Resources", "Practical guides and printable tools for utility setup, internet availability, provider lookup, address changes, outages, and renter responsibilities.", "/resources");
...
export default function ResourcesPage() { const schema = { "@context": "https://schema.org", "@type": "CollectionPage", name: "Moving and utility setup resources", url: `${SITE_URL}/resources`, hasPart: [{ "@type": "CollectionPage", name: "Utility setup guide", url: `${SITE_URL}/resources/utility-setup` }, ...resourceGuides.map((guide) => ({ "@type": "Article", name: guide.h1, url: `${SITE_URL}${guide.path}` })), ...printables.map((item) => ({ "@type": "WebPage", name: item.title, url: `${SITE_URL}/resources/printables/${item.slug}` }))] }; return <main id="main-content"><JsonLd data={schema} /><div className="shell page-breadcrumb-wrap"><Breadcrumbs items={[{ label: "Home", href: "/" }, { label: "Resources" }]} /></div><PageHero eyebrow="Practical library" title="Direct answers. Useful checklists. No filler." description="Start with the task in front of you, then continue to an official provider or government source when the address matters." /><section className="section"><div className="shell resource-hub-callout"><div><span className="eyebrow">Start here</span><h2>Organize every utility setup task.</h2><p>Find electric, water, internet, and trash providers; plan transfers; prepare documents; and choose renter or homeowner steps.</p></div><Link className="button" href="/resources/utility-setup">Open the utility setup hub <ArrowRight size={16} aria-hidden="true" /></Link></div><div className="shell resource-columns">{groups.map(([title, guides]) => <section key={title}><h2>{title}</h2>{guides.map((guide) => <Link href={guide.path} key={guide.path}><span>{guide.h1}</span><ArrowRight size={15} /></Link>)}</section>)}</div></section><section className="section subtle"><div className="shell"><SectionIntro eyebrow="Print and use" title="Checklists that work away from the screen." text="Clean layouts, no sign-in, and no personal information sent to MoveIn." /><div className="printable-grid">{printables.map((item) => <Link href={`/resources/printables/${item.slug}`} key={item.slug}><Download size={21} aria-hidden="true" /><small>{item.audience}</small><h3>{item.title}</h3><p>{item.description}</p><span>Open printable <ArrowRight size={15} /></span></Link>)}</div></div></section><section className="section"><div className="shell"><SectionIntro eyebrow="Official tools" title="Go straight to the source." /><div className="official-tool-row">{official.map((item) => <a href={item.url} target="_blank" rel="noopener noreferrer" key={item.url}><small>{item.organization}</small><strong>{item.title}</strong><span>{item.note}</span><em>Open official site <ExternalLink size={14} /></em></a>)}</div></div></section></main>; }
