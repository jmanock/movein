...
export default async function PrintablePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main id="main-content" className="printable-page"><JsonLd data={schema} /><div className="shell page-breadcrumb-wrap no-print"><Breadcrumbs items={[{ label: "Home", href: "/" }, { label: "Resources", href: "/resources" }, { label: item.title }]} /></div><article className="shell printable-sheet"><header><span className="eyebrow">MoveIn · {item.audience}</span><h1>{item.title}</h1><p>{item.intro}</p><div className="printable-actions no-print"><PrintButton resourceSlug={item.slug} label="Print this resource" /><span>Free · No email · Uses your browser’s print dialog</span></div><p className="printable-reviewed">Reviewed {formatDate(item.reviewed)}</p></header><div className="printable-columns">{item.sections.map((section) => <section className={section.kind === "fields" ? "printable-fields" : undefined} key={section.title}><h2>{section.title}</h2><ul>{section.items.map((entry) => <li key={entry}><span aria-hidden="true" />{entry}</li>)}</ul></section>)}</div><section className="printable-related no-print"><h2>Use this with</h2><div>{item.related.map((related) => related.href.startsWith("http") ? <a href={related.href} target="_blank" rel="noopener noreferrer" key={related.href}>{related.label} <ExternalLink size={14} aria-hidden="true" /><span className="sr-only"> (opens in a new tab)</span></a> : <Link href={related.href} key={related.href}>{related.label}</Link>)}</div></section><footer className="printable-footer"><strong>MoveIn.guide</strong><span>Confirm providers for the exact address. MoveIn is not a utility company or government agency.</span></footer><p className="no-print printable-back"><Link href="/resources">← Browse all moving resources</Link></p></article></main>;
}
