...
export function LookupResults({ result, relatedAreas = [] }: { result: LookupResult; relatedAreas?: RelatedArea[] }) {
  ...
  return <>
    ...
    <section className="zip-guide-links"><span className="eyebrow">Use the result</span><h2>Guides for the next step</h2><div>{result.county ? <Link href={countyPath(result.county)} data-analytics-county={result.county}>{result.county} County utility guide</Link> : null}<Link href="/resources/find-electric-company">Find your electric company</Link><Link href="/resources/find-water-provider">Confirm water and sewer</Link><Link href="/resources/find-internet-providers">Check internet by address</Link><Link href="/resources/change-your-address">Change your address after moving</Link><Link href="/homeowners/set-up-utilities">Homeowner utility setup</Link><Link href="/renters/set-up-utilities">Renter utility setup</Link><Link href="/data-sources">How MoveIn verifies data</Link></div></section>
