...
const categories = [
  ["electricity", "Zap"], ["water", "Droplets"], ["sewer", "Droplets"], ["natural-gas", "Flame"],
  ["internet", "Wifi"], ["trash-recycling", "Recycle"], ["local-government", "Landmark"],
] as const;
...
export function LookupResults({ result }: { result: LookupResult }) {
  ...
  return <>
    ...
    {result.status === "pending" || count === 0 ? <section className="pending-panel"><h2>We know this ZIP code, but we are still verifying its service providers.</h2><p>MoveIn will not guess. Use the official county or city website while this record is being researched.</p></section> : null}
    <div className="result-groups">{categories.map(([category, icon]) => {
      const providers = result.providers[category] ?? [];
      return <section className="result-group" key={category}><div className="result-group-heading"><h2><span className="result-category-icon"><Icon name={icon} size={21} /></span>{categoryLabels[category]}</h2><span>{providers.length ? `${providers.length} ${providers.length === 1 ? "record" : "records"}` : "Verification pending"}</span></div>{providers.length ? <div className="provider-list">{providers.map((provider) => <ProviderCard key={provider.slug} provider={provider} icon={icon} />)}</div> : <div className="empty-provider"><p>We are still verifying this service for your ZIP code.</p></div>}</section>;
    })}</div>
    ...
    <nav className="result-next-links" aria-label="Next steps"><Link href="/homeowners">Homeowner basics <ArrowRight size={16} /></Link><Link href="/renters">Renter basics <ArrowRight size={16} /></Link><Link href={`/corrections?zip=${result.zipCode}`}>Report incorrect information <ArrowRight size={16} /></Link></nav>
  </>;
}

function ProviderCard({ provider, icon }: { provider: LookupProvider; icon: string }) {
