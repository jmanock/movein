...
export function LookupResults({ result, relatedAreas = [] }: { result: LookupResult; relatedAreas?: RelatedArea[] }) {
  ...
  return <>
    ...
    <div className="result-groups">{groups.map((group) => {
      ...
      return <section className="result-group" id={group.key} key={group.key}><div className="result-group-heading"><h2><span className="result-category-icon"><Icon name={group.icon} size={21} /></span>{group.label}</h2><span>{groupProviders.length ? `${groupProviders.length} ${groupProviders.length === 1 ? "record" : "records"}` : "Verification pending"}</span></div>{group.key === "internet" ? <p className="multiple-provider-note"><strong>Internet availability depends on your exact address and should always be verified with the provider.</strong> Technology and speeds can differ by building or unit.</p> : multiplePossible ? <p className="multiple-provider-note"><strong>Multiple providers may serve this ZIP code.</strong> Confirm the exact address with each official provider.</p> : null}{groupProviders.length ? <div className="provider-list">{groupProviders.map((provider) => <ProviderCard key={provider.slug} provider={provider} icon={group.icon} category={group.key} />)}</div> : <div className="empty-provider"><p>We are still verifying this service for your ZIP code.</p>{localDestination ? <a href={localDestination} target="_blank" rel="noopener noreferrer">Visit local government <ExternalLink size={14} aria-hidden="true" /></a> : <Link href={`/corrections?zip=${result.zipCode}`}>Report a missing provider <ArrowRight size={14} aria-hidden="true" /></Link>}</div>}</section>;
    })}</div>
    <section className="recently-moved"><div className="section-heading compact"><span className="eyebrow">Recently moved?</span><h2>People also moving here often need:</h2><p>Driver’s license and address updates, the trash schedule, power and water setup, internet installation, emergency contacts, and local-government information. These links lead to official sources.</p></div><LocalResourceCards resources={recentlyMoved} /></section>
    ...
    <section className="zip-guide-links"><span className="eyebrow">Use the result</span><h2>Guides for the next step</h2><div>{result.county ? <Link href={countyPath(result.county)} data-analytics-county={result.county}>{result.county} County utility guide</Link> : null}<Link href="/resources/find-electric-company">Find your electric company</Link><Link href="/resources/find-water-provider">Confirm water and sewer</Link><Link href="/resources/find-internet-providers">Check internet by address</Link><Link href="/homeowners/set-up-utilities">Homeowner utility setup</Link><Link href="/renters/set-up-utilities">Renter utility setup</Link><Link href="/data-sources">How MoveIn verifies data</Link></div></section>
