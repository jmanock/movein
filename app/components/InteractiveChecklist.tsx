...
export function InteractiveChecklist({ printable }: { printable: Printable }) {
  ...
  const items = useMemo(() => printable.sections.flatMap((section, sectionIndex) => section.kind === "fields" ? [] : section.items.map((title, itemIndex) => ({ id: `${sectionIndex}-${itemIndex}`, title, phase: section.title }))), [printable]);
