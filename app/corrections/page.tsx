...
export default async function CorrectionsPage({ searchParams }: { searchParams: Promise<{ zip?: string }> }) { const { zip = "" } = await searchParams; return <main id="main-content"><PageHero eyebrow="Corrections" title="Help us correct a service record." description="No account is required. Please do not include account numbers, passwords, or sensitive personal information." /><section className="section"><div className="shell correction-card"><h2>Send the details for review</h2><p>Corrections are reviewed against an official provider or government source before public records change. An email is optional and is used only if the reviewer needs clarification.</p><CorrectionForm initialZip={/^\d{5}$/.test(zip) ? zip : ""} /></div></section></main>; }
