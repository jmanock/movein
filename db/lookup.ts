...
type LocationRow = { zip_code: string; city: string | null; county: string | null; state: string; state_name: string; status: LookupStatus; confidence_status: string | null; jurisdiction_notes: string | null; is_indexable: number; last_verified_at: string | null };
type ProviderRow = { provider_id: number; name: string; slug: string; category_slug: string; description: string | null; official_website: string; provider_type: string | null; start_service_url: string | null; address_check_url: string | null; outage_url: string | null; hours: string | null; technology_type: string | null; service_notes: string | null; coverage_type: CoverageType; coverage_notes: string; confidence_level: string; service_availability: string | null; requires_address_confirmation: number; jurisdiction_notes: string | null; is_verified: number; last_verified_at: string | null };
...
export function getLookupResult(zipCode: string): LookupResult | null {
  ...
  const providerRows = database.prepare(`SELECT p.id AS provider_id, p.name, p.slug, pc.slug AS category_slug, p.description, p.official_website, p.provider_type, p.start_service_url, p.address_check_url, p.outage_url, p.hours, p.technology_type, p.service_notes,
  ...
  for (const row of providerRows) {
    ...
    (providers[row.category_slug] ??= []).push({
      ...
      outageUrl: row.outage_url, hours: row.hours, technologyType: row.technology_type,
  ...
  const hasWaterContext = verifiedCategories.has("water") || verifiedCategories.has("sewer");
  const derivedStatus: LookupStatus = providerRows.length === 0 ? "pending" : hasLocalContext && hasElectric && hasWaterContext ? "verified" : "partial";
  return {
    ...
    jurisdictionNotes: location.jurisdiction_notes, isIndexable: Boolean(location.is_indexable) && derivedStatus === "verified", providers,
