...
const rows = database.prepare(`SELECT z.zip_code, ci.name AS city,
  ...
  SUM(CASE WHEN pc.slug='natural-gas' AND COALESCE(p.provider_type,'')!='official_lookup' THEN 1 ELSE 0 END) AS natural_gas
  FROM zip_codes z LEFT JOIN cities ci ON ci.id=z.primary_city_id LEFT JOIN service_areas sa ON sa.zip_code_id=z.id
  LEFT JOIN providers p ON p.id=sa.provider_id LEFT JOIN provider_categories pc ON pc.id=p.category_id
...
const missingPhones = database.prepare(`SELECT p.slug, p.name FROM providers p LEFT JOIN provider_contacts pc ON pc.provider_id=p.id WHERE p.status!='inactive' AND COALESCE(p.provider_type,'')!='official_lookup' GROUP BY p.id HAVING COUNT(pc.id)=0`).all();
